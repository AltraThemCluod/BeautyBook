    ...
    if (getSmsMarketingidatob > 0) {
        $.ajax({
            ...
            success: function (result) {
                ...
                var ServicesIdsarr = result.Item.ServicesId;
                var ServicesIdsarrsplit = ServicesIdsarr.split(',');
                $('#lookupserviceslist').selectpicker('val', ServicesIdsarrsplit);
                $('.selectpicker').selectpicker("refresh");
                $('#emailtemplate').summernote('code', decodeURIComponent(result.Item.SmsTemplate));
