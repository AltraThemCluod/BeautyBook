...
var TaxInvoiceList = function () {
    ...
    let initTaxInvoiceList = function () {
        ...
        $.ajax({
            ...
            serverSide: true,
            type: 'POST',
            url: APIEndPoint + `api/orders/SalonOrderInvoice_All?SalonId=${atob(SalonId)}&InvoiceDate=${IInvoiceDate}&InvoiceNumber=${IInvoiceNumber}&CustomerId=${ICustomerName == "" ? 0 : ICustomerName}`,
            ...
            success: function (Values) {
                ...
                $('#TaxInvoice').DataTable({
                    ...
                    columns: [
                        ...
                        {
                            "title": "" + Langaugestore.Saller_Name +"", "data": "",
                            "render": function (data, type, row) {
                                let htmlData = "";
                            ...
                            "render": function (data, type, row) {
                                ...
                                htmlData = `<div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    ${PermissionData.find(record => record.ModuleKey === "TaxViewInvoice").Value == true ?
                                                    `<a class="dropdown-item" href="/Invoices/TaxInvoiceDetails?OrderId=${btoa(row["Id"])}">
...
function InvoiceCustomers() {
    ...
    $.ajax({
        type: 'POST',
        url: APIEndPoint + `api/orders/SalonOrderVendor_All?SalonId=${atob(SalonId)}`,
        headers: { 'Content-Type': 'application/json', "Authorization": '' + DeviceTokenNumber + '' },
        dataType: 'json',
