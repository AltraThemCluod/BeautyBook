...
function ResetSiplifedInvoice() {
    ...
    $('#PosInvoiceDate').val("");
    $('#PosInvoiceNumber').val(null);
    $('#PosCustomerName').val(0);
...
var SiplifedInvoiceList = function () {
    let initSiplifedInvoiceList = function () {
        ...
        var PosCustomerName = $("#PosCustomerName").val();
        var PosInvoiceNumber = $("#PosInvoiceNumber").val();
        var PosInvoiceDate = $("#PosInvoiceDate").val();

        $.ajax({
            processing: true,
            serverSide: true,
            type: 'POST',
            url: APIEndPoint + `api/pOSPayment/PosInvoice_All?SalonId=${atob(SalonId)}&InvoiceDate=${PosInvoiceDate}&InvoiceNumber=${PosInvoiceNumber}&CustomerId=${PosCustomerName == "" ? 0 : PosCustomerName}`,
                ...
                $('#SiplifedInvoice').DataTable({
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            "render": function (data, type, row) {
                                let htmlData = "";
                                htmlData = `#${row["PosInvoiceNumber"] == "" || row["PosInvoiceNumber"] == null ? '-' : row["PosInvoiceNumber"]}`;
                                return htmlData;
                            }
                            , "orderable": false, "width": "1%"
                        },
                        {
                            ...
                            "render": function (data, type, row) {
                                let htmlData = "";
                                htmlData = `${row["InvoiceDateStr"] == "" && row["InvoiceDateStr"] == null ? '-' : row["InvoiceDateStr"]}`;
                                return htmlData;
                            }
                            , "orderable": false, "width": "1%"
                        },
                        ...
                        },
                        {
                            ...
                            "render": function (data, type, row) {
                                let htmlData = "";
                                htmlData += `${row["EnteredByUserName"] == "" && row["EnteredByUserName"] == null ? '-' : row["EnteredByUserName"]}`;
                                return htmlData;
                            }
                            , "orderable": false, "width": "1%"
                        },
                            ...
                            "render": function (data, type, row) {
                                ...
                                htmlData = `<div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    ${row["ParentId"] > 0 ? 
                                                        '' : `${PermissionData.find(record => record.ModuleKey === "POSNoticeCreditor").Value == true ? `<a class="dropdown-item" href="javascript:void(0)" onclick="NoticeCreditor('${btoa(row["Id"])}');"><i class="bb-credit-card text-gray-600 fs-16 mr-3"></i>${Langaugestore.Notice_Creditor}</a>` : ""}`
                                                    }
                                                   ${PermissionData.find(record => record.ModuleKey === "POSViewInvoice").Value == true ?
                                                    `<a class="dropdown-item" href="/Pos/PosInvoice?ri=${btoa(row["Id"])}">
