...
function GeneratAppoinmentInvoice(AppoinementInvoiceId) {
    debugger
    window.location.href = `/Appointments/Invoice?InvoiceId=${btoa(AppoinementInvoiceId)}`
}
...
function AppoinmentInvoice() {
    ...
    $.ajax({
        type: 'POST',
        url: APIEndPoint + `/api/userAppointments/AppoinmentInvoice_ById?Id=${getInvoiceIdatob}`,
        ...
        success: function (result) {
            ...
            //Appointment invoice ZATCA Qr code image set
            if (result.Item.QrCodeURL != null || result.Item.QrCodeURL != "") {
                $("#qrcodeUrl").attr("src", APIEndPoint + "/SimplifiedInvoice/" + result.Item.QrCodeURL);
            } else {
                $("#qrcodeUrl").hide();
            }
