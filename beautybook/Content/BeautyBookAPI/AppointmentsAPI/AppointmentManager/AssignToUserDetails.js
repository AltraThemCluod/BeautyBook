...
function appoinmentModaldetails(AssignToUserId) {
    $('#employeeInfodetails').html(``);
    $('#employeeWorksheet').html(``);
    $('#profileModalbtn').html(``);
    $('#userServicestable').html(``);
    $('#EmployeeDetailsModalbtn').html(``);

    $("#employeeProfileloader").show();
    $('#employeeWorksheetloader').show();
    $('#userServicestableloader').show();
    leaveCount(AssignToUserId);
    if (AssignToUserId > 0) {
        $.ajax({
            type: 'POST',
            url: APIEndPoint + `/api/users/Users_ById?Id=${AssignToUserId}&IsLanguage=${Languages}`,
            ...
            success: function (result) {
                            ...
                            <div class="col-lg-12 mb-3">
                                ...
                                <div>
                                    <span class="font-weight-medium">
                                        ${result.Item.AddressLine1 == "" || result.Item.AddressLine1 == null || result.Item.AddressLine2 == "" || result.Item.AddressLine2 == null ? '-' : `${result.Item.AddressLine1} <br/> ${result.Item.AddressLine2}`} <br/>
                                    </span>
                                ...
                                if (categoryId == result.Item.UserServices[j].LookUpCategoryId) {
                                    ...
                                    if (!serviceIds.includes(serviceId) && serviceId > 0) {
                                        ...
                                        serviceName += `
                                                ...
                                                `<span class="badge bg-soft-danger text-danger border p-1" style="cursor:pointer;">In-Active</span>` : ''
                                            }
                                                    </th>
                                                    <td>${result.Item.UserServices[j].Duration} Min</td>
                                            ...
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="bg-soft-primary text-primary">
                                                        ${result.Item.UserServices[i].LookUpServicesId == 0 ?
                                                            `${result.Item.UserServices[i].Duration} Min` : `${Langaugestore.Duration}`
                                                        }
                                                    </th>
                                                    <th class="bg-soft-primary text-primary">
                                                        ${result.Item.UserServices[i].LookUpServicesId == 0 ?
                ...
                $('#EmployeeDetailsModalbtn').append(`
