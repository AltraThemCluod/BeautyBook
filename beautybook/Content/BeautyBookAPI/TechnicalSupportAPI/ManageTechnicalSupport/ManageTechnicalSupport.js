...
//searchTechnicalSupportdata function call
...
var TechnicalSupportList = function () {
    ...
    let initTechnicalSupportList = function () {
        ...
        $.ajax({
            ...
            serverSide: true,
            type: 'POST',
            url: APIEndPoint + `/api/technicalSupport/TechnicalSupport_All?SalonId=${atob(SalonId)}&UserId=${atob(UserId)}`,
                    ...
                    columns: [
                        {
                            ...
                            "render": function (data, type, row) {
                                let htmlData = "";
                                htmlData = `${row["Discription"] == "" || row["Discription"] == null ? '-' : decodeURIComponent(escape(window.atob(row["Discription"])))}`;
                                return htmlData;
                            }
                            , "orderable": false, "width": "40%"
                        },
                        {
                            "title": "" + Langaugestore.Created_Date +"", "data": "",
                            "render": function (data, type, row) {
                                let htmlData = "";
                        ...
                        },
                        {
                            ...
                            "render": function (data, type, row) {
                                let htmlData = "";
                                htmlData = `<td>
                                                <a href="/TechnicalSupport/TechnicalSupportDetails?Id=${btoa(row["Id"])}" style="cursor:pointer;"class="btn btn-light border btn-sm"><i class="bb-edit-3 text-gray-600 fs-16 mr-2"></i>${Langaugestore.EDIT}</a>
                                                <a href="javascript:void(0)" onclick="confirmTicketDeleted(${row["Id"]})" class="btn btn-danger btn-sm"><i class="bb-trash-2 text-white fs-16 mr-2"></i>${Langaugestore.Delete}</a>
                                            </td>`;
                                return htmlData;
                            }
                            , "orderable": false, "width": "10%"
...
function DeleteTicket(ticketId) {
    ...
    $.ajax({
        type: 'POST',
        url:   APIEndPoint + `/api/technicalSupport/TechnicalSupport_Delete?Id=${(ticketId)}&DeletedBy=${(atob(UserId))}`,
